...
class ValueIteration:
    def __init__(self, gamma=0.925, num_rows=5, num_cols=5, theta=1e-4, isCatNipEnabled=False, setCatnipStateAsTerminal=False, useStandardVersion=True):
        self.gamma = gamma
        self.theta = theta
        ...
        self.isCatNipEnabled = isCatNipEnabled
        self.setCatnipStateAsTerminal = setCatnipStateAsTerminal
        self.useStandardVersion = useStandardVersion
        if self.isCatNipEnabled and self.setCatnipStateAsTerminal:
            for state in CATNIP_STATES: 
    ...
    def get_reward(self, curr_state, next_state):
        if curr_state in TERMINAL_STATES and next_state in TERMINAL_STATES:
            return 0
        if self.isCatNipEnabled and next_state in CATNIP_STATES:
            return 5
    ...
    def get_all_possible_next_states_with_transition_probabilities(self, curr_state, action):
        ...
        next_states.append((curr_state, 0.06))
        ...
        next_state = next_state if self.is_valid_state(next_state) else curr_state
        next_states.append((next_state, 0.12))
